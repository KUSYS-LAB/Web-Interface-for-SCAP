...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#pki-zip").on("change", function (e) {
                function handler(f) {
                    JSZip.loadAsync(f)
                        .then(function (zip) {
                            zip.forEach(function (fn) {
                                type = null;
                                if (fn == "privateKey.pem") type = 'string';
                                else if (fn == "publicKey.pem") type = 'string';
                                else if (fn == "certSig.sig") type = 'string';
                                else if (fn == "cert.der") type = 'binarystring';

                                zip.file(fn).async(type)
                                    .then(function (content) {
                                        if (fn == "privateKey.pem") {
                                            localStorage.setItem("${USER }" + "_prv", content);
                                        } else if (fn == "publicKey.pem") {
                                            localStorage.setItem("${USER }" + "_pub", content);
                                        } else if (fn == "certSig.sig") {
                                            localStorage.setItem("${USER }" + "_sig", stohex(content));
                                        } else if (fn == "cert.der") {
                                            localStorage.setItem("${USER }" + "_cert", stob64(content));
                                        }
                                    });
                            });
                        });
...
<div id="wrapper">
    ...
    <div id="content-wrapper">
        ...
        <div class="container-fluid">
            ...
            <h1>Import PKI${USER}</h1>
            <hr>
            <form>
                <div class="form-group">
                    <div class="form-label-group">
                        <input type="file" class="form-control-file border" id="pki-zip" name="pki-zip">
                        <label for="pki">pki.zip to import</label>
                    </div>
                </div>
            </form>
