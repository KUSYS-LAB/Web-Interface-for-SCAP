        ...
        $(document).ready(function () {
            $("#export-button").click(function () {
                ...
                if (seed != null && seed != "") {
                    ...
                    var signature = localStorage.getItem(id + "_sig");

                    var cert = atob(bs64Cert);
                    var sig = new KJUR.crypto.Signature({alg: "SHA256withECDSA"});
                    sig.init(pemPrivateKey, seed);
                    sig.updateString(cert);
                    var sigValueHex = sig.sign();

                    var sigV = new KJUR.crypto.Signature({alg: "SHA256withECDSA"});
                    sigV.init(pemPublicKey);
                    sigV.updateString(cert);
                    var isValid = sigV.verify(sigValueHex);

                    if (isValid) {

                        var privateKeyFile = zip.file("privateKey.pem", pemPrivateKey);
                        var publicKeyFile = zip.file("publicKey.pem", pemPublicKey);
                        var sigFile = zip.file("certSig.sig", hextorstr(signature));
                        var certFile = zip.file("cert.der", cert, {binary: true});


                        zip.generateAsync({type: "blob"})
                            .then(function (blob) {
                                saveAs(blob, id + "_pki.zip");
                            }, function (err) {
                                console.log(err);
                            });
                    } else {
...
<div id="wrapper">
    ...
    <div id="content-wrapper">
        ...
        <div class="container-fluid">
            ...
            <h1>Export PKI${USER}</h1>
            <hr>
            <form>
                <div class="form-group">
                    <div class="form-label-group">
                        <label for="passphrase">Pass phrase</label>
                        <input type="password" class="form-control" name="seed-value" id="seed-value"
                               placeholder="Enter Passphrase" required="required">
                    </div>
                    <input type="button" class="btn btn-primary" value="Export" id="export-button">
                </div>
            </form>
